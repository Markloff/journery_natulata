...
function isVersionValid(currentVersion, date, requestedVersion, notices = []) {
    ...
    if (!desiredVersion) {
        notices.push(`Could not parse engines.vscode value ${requestedVersion}. Please use, for example: ^1.22.0, ^1.22.x, etc.`);
        return false;
    }
    ...
    if (desiredVersion.majorBase === 0) {
        ...
        if (!desiredVersion.majorMustEqual || !desiredVersion.minorMustEqual) {
            notices.push(`Version specified in engines.vscode (${requestedVersion}) is not specific enough. For vscode versions before 1.0.0, please define at a minimum the major and minor desired version. E.g. ^0.10.0, 0.10.x, 0.11.0, etc.`);
            return false;
        }
    }
    else {
        ...
        if (!desiredVersion.majorMustEqual) {
            notices.push(`Version specified in engines.vscode (${requestedVersion}) is not specific enough. For vscode versions after 1.0.0, please define at a minimum the major desired version. E.g. ^1.10.0, 1.10.x, 1.x.x, 2.x.x, etc.`);
            return false;
        }
    }
    if (!isValidVersion(currentVersion, date, desiredVersion)) {
        notices.push(`Extension is not compatible with Code ${currentVersion}. Extension requires: ${requestedVersion}.`);
        return false;
    }
    return true;
}
